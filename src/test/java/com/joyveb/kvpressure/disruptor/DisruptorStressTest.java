...
public class DisruptorStressTest {
	...
	public void shouldHandleLotsOfThreads() throws Exception {
		Disruptor<TestEvent> disruptor = new Disruptor<TestEvent>(
				TestEvent.FACTORY, 1 << 16, executor, ProducerType.MULTI,
		...
		TestEventHandler[] handlers = initialise(disruptor,
				new TestEventHandler[handlerCount]);
		Publisher[] publishers = initialise(new Publisher[publisherCount],
				ringBuffer, iterations, barrier, latch);
	...
	private Publisher[] initialise(Publisher[] publishers,
			RingBuffer<TestEvent> buffer, int messageCount,
	...
	private TestEventHandler[] initialise(Disruptor<TestEvent> disruptor,
			TestEventHandler[] testEventHandlers) {
	...
	private static class TestEventHandler implements EventHandler<TestEvent> {
		...
		public void onEvent(TestEvent event, long sequence, boolean endOfBatch)
				throws Exception {
			if (event.sequence != sequence || event.a != sequence + 13
					|| event.b != sequence - 7
	...
	private static class Publisher implements Runnable {
		...
		public Publisher(RingBuffer<TestEvent> ringBuffer, int iterations,
