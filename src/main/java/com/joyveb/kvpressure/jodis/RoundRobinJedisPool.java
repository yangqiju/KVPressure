...
public class RoundRobinJedisPool implements JedisResourcePool {
	 ...
	 */
	@Deprecated
	public RoundRobinJedisPool(String zkAddr, int zkSessionTimeoutMs,
			String zkPath, JedisPoolConfig poolConfig) {
		this(zkAddr, zkSessionTimeoutMs, zkPath, poolConfig,
	...
	@Deprecated
	public RoundRobinJedisPool(String zkAddr, int zkSessionTimeoutMs,
			String zkPath, JedisPoolConfig poolConfig, int timeout) {
		this(CuratorFrameworkFactory
				...
				.retryPolicy(
						new BoundedExponentialBackoffRetryUntilElapsed(
								CURATOR_RETRY_BASE_SLEEP_MS,
								CURATOR_RETRY_MAX_SLEEP_MS, -1L)).build(),
				true, zkPath, poolConfig, timeout);
	}
	 ...
	 */
	@Deprecated
	public RoundRobinJedisPool(CuratorFramework curatorClient,
			boolean closeCurator, String zkPath, JedisPoolConfig poolConfig) {
		this(curatorClient, closeCurator, zkPath, poolConfig,
				Protocol.DEFAULT_TIMEOUT);
	}
	...
	public RoundRobinJedisPool(CuratorFramework curatorClient,
			boolean closeCurator, String zkPath, JedisPoolConfig poolConfig,
			int timeout) {
		this(curatorClient, closeCurator, zkPath, poolConfig, timeout, timeout,
				null, Protocol.DEFAULT_DATABASE, null);
	}

	private RoundRobinJedisPool(CuratorFramework curatorClient,
			boolean closeCurator, String zkProxyDir,
			JedisPoolConfig poolConfig, int connectionTimeoutMs,
			int soTimeoutMs, String password, int database, String clientName) {
		...
		watcher.getListenable().addListener(new PathChildrenCacheListener() {
			...
			public void childEvent(CuratorFramework client,
				...
				StringBuilder sb = new StringBuilder(
						"zookeeper event received: type=").append(event
						.getType());
				if (event.getData() != null) {
					ChildData data = event.getData();
					sb.append(", path=").append(data.getPath())
							.append(", stat=").append(data.getStat());
				}
	...
	private void resetPools() {
		...
		Map<String, PooledObject> addr2Pool = Maps
				.newHashMapWithExpectedSize(pools.size());
		...
		for (ChildData childData : watcher.getCurrentData()) {
			try {
				CodisProxyInfo proxyInfo = MAPPER.readValue(
						childData.getData(), CodisProxyInfo.class);
				...
				if (pool == null) {
					...
					pool = new PooledObject(addr, new JedisPool(poolConfig,
							host, port, connectionTimeoutMs, soTimeoutMs,
	...
	@Override
	public Jedis getResource(int id) {
		ImmutableList<PooledObject> pools = this.pools;
		if (pools.isEmpty()) {
			throw new JedisException("Proxy list empty");
		}
		return pools.get(id % pools.size()).pool.getResource();
	}
	...
	public void close() {
		try {
			...
			throw new AssertionError("IOException should not have been thrown",
					e);
		}
		if (closeCurator) {
			curatorClient.close();
	...
	public static final class Builder {
		 ...
		 */
		public Builder curatorClient(CuratorFramework curatorClient,
				boolean closeCurator) {
			this.curatorClient = curatorClient;
			this.closeCurator = closeCurator;
			return this;
		}
		...
		private void validate() {
			Preconditions
					.checkNotNull(zkProxyDir, "zkProxyDir can not be null");
		...
		public RoundRobinJedisPool build() {
			validate();
			return new RoundRobinJedisPool(curatorClient, closeCurator,
					zkProxyDir, poolConfig, connectionTimeoutMs, soTimeoutMs,
					password, database, clientName);
		}
	}
}
